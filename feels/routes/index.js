...
router.post('/deleteAll', function(req, res, next) {
    ...
    Comment.deleteMany({},function(err, post) { 
      if (err) return console.error(err);
      else{
      res.sendStatus(200);
      }
...
router.get('/comment', function(req, res, next) {
    ...
    Comment.find(function(err,commentList) { //Calls the find() method on your database
      if (err) return console.error(err); //If there's an error, print it out
        ...
        res.json(commentList); //Then send the comment
...
router.get('/byUser', function(req, res, next) {
    console.log("get by user route");
    console.log(req.query.q)
    
    Comment.find({Name:req.query.q}, function (err, docs) {
